...
def pairwise_distance_diff(pc0, pc1):
  """Compute pairwise distance of a point cloud.
  ...
  Returns:
    pairwise distance: (batch_size, num_points, num_points)
  """
  og_batch_size = pc0.get_shape().as_list()[0]
  point_cloud = tf.squeeze(pc0)
  point_cloud1 = tf.squeeze(pc1)
  if og_batch_size == 1:
    point_cloud = tf.expand_dims(point_cloud, 0)
    point_cloud1 = tf.expand_dims(point_cloud1, 0)
    
  point_cloud_transpose = tf.transpose(point_cloud1, perm=[0, 2, 1])
  point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)
  point_cloud_inner = -2*point_cloud_inner
  point_cloud_square = tf.reduce_sum(tf.square(point_cloud), axis=-1, keep_dims=True)
...
def pairwise_distance2(point_cloud):
    og_batch_size = point_cloud.get_shape().as_list()[0]
    point_cloud = tf.squeeze(point_cloud)
    if og_batch_size == 1:
        point_cloud = tf.expand_dims(point_cloud, 0)

    pts_num = tf.shape(point_cloud)[1]
    features1_1 = tf.tile(tf.expand_dims(point_cloud, axis=-2), [1, 1, pts_num, 1])
    features1_2 = tf.tile(tf.expand_dims(point_cloud, axis=-3), [1, pts_num, 1, 1])
    features1 = features1_1 - features1_2
    features2 = tf.reduce_sum(tf.square(features1), axis=-1, keep_dims=False)
    return features2

def pairwise_distance4(point_cloud):
  """Compute pairwise distance of a point cloud.

  Args:
    point_cloud: tensor (batch_size, num_points, k_points, num_dims)

  Returns:
    pairwise distance: (batch_size, num_points, k_points, k_points)
  """
  og_batch_size = point_cloud.get_shape().as_list()[0]
  point_cloud = tf.squeeze(point_cloud)
  if og_batch_size == 1:
  ...
  point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 1, 3, 2])
  point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)
  point_cloud_inner = -2*point_cloud_inner
  point_cloud_square = tf.reduce_sum(tf.square(point_cloud), axis=-1, keep_dims=True)
  point_cloud_square_tranpose = tf.transpose(point_cloud_square, perm=[0, 1, 3, 2])
  return point_cloud_square + point_cloud_inner + point_cloud_square_tranpose
